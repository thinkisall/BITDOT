  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 sm:py-6">
        ...
        {/* MA 사이클 스캐너 배너 */}
        <div className="mb-4 sm:mb-6 rounded-xl border border-purple-500/20 bg-linear-to-r from-purple-950/20 to-zinc-900 p-4 sm:p-5">
          <div className="flex items-start sm:items-center gap-3 sm:gap-4 flex-col sm:flex-row">
            <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-purple-500/15 flex items-center justify-center shrink-0">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm sm:text-base font-bold text-white mb-1">
                <span className="text-purple-400">MA 사이클</span> 스캐너
              </div>
              <p className="text-[10px] sm:text-xs text-zinc-400 leading-relaxed">
                1분봉에서 MA50·110·180이{' '}
                <strong className="text-zinc-300">수렴 → 정배열 → 재수렴</strong> 후{' '}
                현재 <strong className="text-red-400">역배열 확산 중</strong>인 종목을 포착합니다.
                빗썸 · Bybit 전종목 스캔.
              </p>
              <div className="flex flex-wrap gap-2 mt-2">
                {['수렴→정배열→수렴→역배열', '1분봉 사이클 탐지', '빗썸 + Bybit'].map((text) => (
                  <span key={text} className="text-[9px] sm:text-[10px] px-2 py-0.5 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/20">
                    {text}
                  </span>
                ))}
              </div>
            ...
            <Link
              href="/ma-cycle"
              className="shrink-0 px-4 py-2 bg-purple-700 hover:bg-purple-600 text-white font-bold rounded-lg transition-colors text-xs sm:text-sm whitespace-nowrap"
            >
              분석 보기
            </Link>
          </div>
