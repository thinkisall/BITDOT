...
export async function GET() {
  try {
    ...
    const allData: FundingData[] = [...binanceData, ...bybitData];
...
async function fetchOKXFunding(): Promise<FundingData[]> {
  try {
    ...
    const fundingResponse = await fetch(
      "https://www.okx.com/api/v5/public/funding-rate-history?instType=SWAP&limit=100",
    );
    ...
    if (fundingData.data.length > 0) {
      console.log(
        "OKX funding sample data:",
        JSON.stringify(fundingData.data[0]),
      );
    }
